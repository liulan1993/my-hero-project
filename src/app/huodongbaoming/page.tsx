            ...
            {progress < 100 ? (
                ...
                >
                    <motion.div
                        ...
                        style={{ background: 'linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.5), transparent)', filter: 'blur(4px)' }}
                        animate={{ x: ['-100%', '1000%'] }}
    ...
    return (
        <div className="text-gray-300 text-center">
            {lines.map((line, index) => {
                if (line.startsWith('# ')) return <h1 key={index} className="text-3xl font-bold my-4 text-white">{line.substring(2)}</h1>;
                if (line.startsWith('## ')) return <h2 key={index} className="text-2xl font-bold my-3 text-white">{line.substring(3)}</h2>;
                if (line.startsWith('### ')) return <h3 key={index} className="text-xl font-bold my-2 text-white">{line.substring(4)}</h3>;
                if (line === '') return <br key={index} />;
                if (line.includes('报名截止')) return <p key={index} className="my-2 leading-relaxed font-semibold text-purple-300">{line}</p>;
    ...
    return (
      ...
      >
        ...
        <motion.div
          ...
          style={{ background: `radial-gradient(ellipse at bottom right, rgba(172, 92, 255, 0.7) -10%, rgba(79, 70, 229, 0) 70%), radial-gradient(ellipse at bottom left, rgba(56, 189, 248, 0.7) -10%, rgba(79, 70, 229, 0) 70%)`, filter: "blur(40px)" }}
...
const ExpandedCard = ({ product, onCollapse }: { product: Product; onCollapse: () => void; }) => {
    return (
      <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onCollapse}>
        <motion.div
        ...
        >
          <div className="relative flex-shrink-0 p-8 pb-0 z-20">
             <motion.button initial={{opacity: 0, scale: 0.5}} animate={{opacity: 1, scale: 1}} transition={{delay: 0.3}} onClick={onCollapse} className="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors w-8 h-8 bg-white/10 rounded-full flex items-center justify-center z-50" aria-label="Close article">✕</motion.button>
             <div className="w-12 h-12 rounded-full flex items-center justify-center mb-6 bg-gradient-to-tr from-[#171c2c] to-[#121624]" style={{ boxShadow: "0 8px 16px -2px rgba(0, 0, 0, 0.3), 0 4px 8px -1px rgba(0, 0, 0, 0.2), inset 2px 2px 5px rgba(255, 255, 255, 0.15), inset -2px -2px 5px rgba(0, 0, 0, 0.7)"}}>
                <svg width="20" height="20" viewBox="0 0 16 16" fill="white" xmlns="http://www.w3.org/2000/svg"><path d="M8 0L9.4 5.4L14.8 5.4L10.6 8.8L12 14.2L8 10.8L4 14.2L5.4 8.8L1.2 5.4L6.6 5.4L8 0Z" /></svg>
            </div>
          </div>
...
const CollisionMechanism = ({ parentRef, beamOptions = {} }: { parentRef: React.RefObject<HTMLDivElement | null>; beamOptions?: { initialX?: number; translateX?: number; initialY?: number; translateY?: number; rotate?: number; className?: string; duration?: number; delay?: number; repeatDelay?: number; }; }) => {
  ...
  useEffect(() => { setTargetY(window.innerHeight + 200); }, []);
  ...
  useEffect(() => {
    ...
    const checkCollision = () => {
      if (beamRef.current && parentRef.current) {
        ...
        if (beamRect.bottom >= window.innerHeight && !collision.detected) {
    ...
    // 优化：将检测间隔从 50ms 增加到 150ms
    const animationInterval = setInterval(checkCollision, 150);
    return () => clearInterval(animationInterval);
  }, [collision.detected, parentRef, targetY]);

  return (
    <>
      <motion.div key={beamKey} ref={beamRef} animate="animate" initial={{ y: beamOptions.initialY || -200, x: beamOptions.initialX || 0, rotate: beamOptions.rotate || 0 }} variants={{ animate: { y: targetY, x: beamOptions.translateX || 0, rotate: beamOptions.rotate || 0 } }} transition={{ duration: beamOptions.duration || 8, repeat: Infinity, repeatType: "loop", ease: "linear", delay: beamOptions.delay || 0, repeatDelay: beamOptions.repeatDelay || 0 }} className={cn("absolute left-0 top-20 m-auto h-14 w-px rounded-full bg-gradient-to-t from-indigo-500 via-purple-500 to-transparent", beamOptions.className)} />
      <AnimatePresence>
...
// --- 3D 场景代码 (核心性能优化) ---
const AnimatedInstancedBoxes = () => {
    const COUNT = 50;
    const ref = useRef<THREE.InstancedMesh>(null!);
    const groupRef = useRef<THREE.Group>(null!);
    const dummy = useMemo(() => new THREE.Object3D(), []);

    // 优化: 使用 InstancedMesh 仅创建一次几何体和材质
    const shape = useMemo(() => {
    ...
    const extrudeSettings = useMemo(() => ({ depth: 0.3, bevelEnabled: true, bevelThickness: 0.05, bevelSize: 0.05, bevelSegments: 2, curveSegments: 8 }), []);
    const geometry = useMemo(() => new THREE.ExtrudeGeometry(shape, extrudeSettings).center(), [shape, extrudeSettings]);

    // 初始化每个实例的位置和旋转
    useEffect(() => {
        for (let i = 0; i < COUNT; i++) {
            dummy.position.set((i - COUNT / 2) * 0.75, 0, 0);
            dummy.rotation.set((i - 10) * 0.1, Math.PI / 2, 0);
            dummy.updateMatrix();
    ...
    return (
        <group ref={groupRef}>
            <instancedMesh ref={ref} args={[geometry, undefined, COUNT]}>
                {/* 优化：使用更轻量的材质 */}
                <meshStandardMaterial color="#232323" metalness={1} roughness={0.3} />
            </instancedMesh>
        </group>
    );
};
